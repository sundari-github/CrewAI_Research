...
**Add your `OPENAI_API_KEY` into the `env.config` file**

Add these values to `env.config` for the email task:
...
The application now loads environment variables from `env.config` at startup.
The email task also requires the SMTP settings above to be present.
